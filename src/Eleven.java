...
public class Eleven // this is the least inspiring of the problems thus far.
{
    public static int[][] readGrid(String filename, int size)
    {
        ...
        try {
            ...
            Scanner numReader = new Scanner(bufferedReader);

            for(int row = 0; row < size; row++){
                for(int col = 0; col < size; col++){
                    results[row][col] = numReader.nextInt();
                }
            }
            numReader.close();
        }
    ...
    public static void main(String[] args)
    {
        ...
        for(int i = 0; i < 20; i++){
            for(int j = 0; j < 16; j++){
                ...
                currentProduct = matrix[j][i] * matrix[j+1][i] * matrix[j+2][i] * matrix[j+3][i];
                if(currentProduct > max){
                    max = currentProduct;
                }
            }
